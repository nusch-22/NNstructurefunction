...
def load_best_parameters(name):
    """Load best parameters from file generated by run_hyperopt."""
...
def load_pred_grids():
    """Load the theoretical prediction (x,Q^2)-grids."""
...
def train_model(data_df, rep_id, best_params, name):
    """Train the model using the best parameters in one replica with ID rep_id."""
...
def fit_replica(data_df, rep_id, model, name):
    """Use the model to fit to replica rep_id and save the fit to a file."""
...
def high_pred_replica(pred_df, rep_id, model, name):
    """Use the model to make interpolated predictions for replica rep_id and save the predictions to a file."""
    # make prediction looping over x values
    x_set = set(pred_df["x"])
    n_grid = 100
    data_pred = np.empty((len(x_set), n_grid, 3))
