...
def filter_NMCPD():
    ...
    for i in tables:
        hepdata_tables = (
            "rawdata/HEPData-ins426595-" + version + "-Table_" + str(i) + ".yaml"
        )
        ...
        for j in range(len(values)):
            ...
            y = Q2 / (sqrts * sqrts * x)
            kin_value = {
                "x": {"min": None, "mid": x, "max": None},
                "q2": {"min": None, "mid": Q2, "max": None},
            ...
            error_value_nuc = {
                "nuclear": input["dependent_variables"][0]["values"][j]["errors"][0][
                    "symerror"
                ]
            }
