...
def plot_scans(df, bestdf, trials, bestid, filename):
    ...
    f, axs = plt.subplots(1, nplots, sharey=False, figsize=(50, 7))

    loss = df.get("loss")
    mask = np.where(loss < 0.3)[0]

    axs[0].scatter(df.get("iteration")[mask], df.get("loss")[mask])
...
def get_model_with_loss(trials, loss):
    ...
    losses_path = current_path / folder
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "trials", help="Take as input a .pickle or .json file with trials."
    )
